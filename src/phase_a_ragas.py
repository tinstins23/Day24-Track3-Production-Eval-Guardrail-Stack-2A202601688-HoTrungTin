...
def _metric(pq, name: str) -> float:
    if hasattr(pq, name):
        return float(getattr(pq, name) or 0.0)
    if isinstance(pq, dict):
        return float(pq.get(name, 0.0) or 0.0)
    return 0.0
...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    for a, pq in zip(answers, per_q):
        results.append(RagasResult(
            ...
            faithfulness=_metric(pq, "faithfulness"),
            answer_relevancy=_metric(pq, "answer_relevancy"),
            context_precision=_metric(pq, "context_precision"),
            context_recall=_metric(pq, "context_recall"),
        ))
    return results
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    dist_names = ["factual", "multi_hop", "adversarial"]
    avg_by_dist = {}
    for d in dist_names:
        subset = [r for r in results if r.distribution == d]
        avg_by_dist[d] = (
            sum(r.avg_score for r in subset) / len(subset) if subset else 1.0
        )

    dominant_dist = max(
        dist_names,
        key=lambda d: (sum(matrix[m][d] for m in matrix), -avg_by_dist[d]),
    )
    dominant_metric = max(matrix, key=lambda m: sum(matrix[m].values()))
    insight = (
        f"Distribution '{dominant_dist}' có nhiều failure nhất "
        f"(avg_score={avg_by_dist[dominant_dist]:.3f}). "
